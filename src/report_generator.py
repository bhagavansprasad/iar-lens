...
def _build_what_it_does(flow_context: dict) -> str:
    """Section 2 — What This Integration Does (from flow_context.integration_purpose)"""
    if not flow_context:
        return ""

    purpose = flow_context.get("integration_purpose", "").strip()
    if not purpose:
        return ""

    change_type   = flow_context.get("change_type", "").replace("_", " ").title()
    change_reason = flow_context.get("change_type_reason", "").strip()
    ...
    if added or removed:
        parts = []
        if added:
            parts.append(f"**Added:** {', '.join(f'`{a}`' for a in added)}")
        if removed:
            parts.append(f"**Removed:** {', '.join(f'`{r}`' for r in removed)}")
        adapters_line = f"\n\n**Adapter Changes:** {' | '.join(parts)}"

    change_type_line = ""
    if change_type:
        change_type_line = f"\n\n**Change Classification:** `{change_type}`"
        if change_reason:
            change_type_line += f" — {change_reason}"
...
def _build_what_changed(flow_context: dict) -> str:
    """Section 3 — What Changed (from flow_context narrative fields)"""
    if not flow_context:
        return ""

    flow_before     = flow_context.get("flow_before", "").strip()
    flow_after      = flow_context.get("flow_after", "").strip()
    change_narrative= flow_context.get("change_narrative", "").strip()
    ...
    if flow_before:
        parts.append(f"**Before (v{flow_context.get('version_from', '')}):**\n{flow_before}\n")

    if flow_after:
        parts.append(f"**After (v{flow_context.get('version_to', '')}):**\n{flow_after}\n")
...
def generate_report(
    ...
    # Load flow_context if available — <label>_flow_context.json or flow_context.json
    flow_context = None
    label        = getattr(config, "LABEL", None)
    output_dir   = os.path.dirname(delta_path)
    for candidate in filter(None, [
        os.path.join(output_dir, f"{label}_flow_context.json") if label else None,
        os.path.join(output_dir, "flow_context.json"),
    ]):
        if os.path.exists(candidate):
            with open(candidate, "r", encoding="utf-8") as f:
                flow_context = json.load(f)
            logger.info(f"Loaded flow context: {candidate}")
            break
    if not flow_context:
        logger.warning("No flow_context.json found — narrative sections will be omitted")
